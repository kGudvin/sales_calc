...
- внешняя Docker-сеть `web` и reverse proxy для публикации приложения
...
Команду создания общей сети для reverse proxy достаточно выполнить один раз; если сеть уже существует, этот шаг пропускается.
...
Контейнер приложения не публикует порт напрямую на сервере. Reverse proxy должен направлять запросы на:

```text
sales-calc-app:8090
```

Пользователь открывает домен, настроенный в reverse proxy, например:

```text
https://calc.example.com
```

Для локальной разработки без reverse proxy можно добавить отдельный Compose override с пробросом `8090:8090`.
...
- фильтр по типу;
- автоматическое название по дате, заказчику или ИНН;
- адаптивный интерфейс с кнопкой добавления товара под списком;
...
Хранятся последние 14 дней. Более старые файлы удаляются автоматически. Файл сначала создаётся как временный, проверяется и только затем атомарно переименовывается; права доступа ограничены владельцем.
...
Импорт и хранение прайсов доступны администратору. Поиск по прайсам в карточке расчёта временно скрыт из интерфейса.
